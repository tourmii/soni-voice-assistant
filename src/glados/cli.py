...
MODEL_CHECKSUMS = {
    ...
    "models/ASR/zipformer/encoder-epoch-12-avg-8.onnx": "",  # Will be calculated during download
    "models/ASR/zipformer/decoder-epoch-12-avg-8.onnx": "",  # Will be calculated during download
    "models/ASR/zipformer/joiner-epoch-12-avg-8.onnx": "",   # Will be calculated during download
    "models/ASR/zipformer/tokens.txt": "",                   # Will be calculated during download
    "models/ASR/zipformer/bpe.model": "",                    # Will be calculated during download
    "models/TTS/glados.onnx": "17ea16dd18e1bac343090b8589042b4052f1e5456d42cad8842a4f110de25095",
    "models/TTS/vi_VN-vais1000-medium.onnx": "",                        # Will be calculated during download
    "models/TTS/vi_VN-vais1000-medium.onnx.json": "",                   # Will be calculated during download
...
MODEL_URLS = {
    ...
    "models/ASR/zipformer/encoder-epoch-12-avg-8.onnx": "https://huggingface.co/csukuangfj/sherpa-onnx-zipformer-vi-2025-04-20/resolve/main/encoder-epoch-12-avg-8.onnx",
    "models/ASR/zipformer/decoder-epoch-12-avg-8.onnx": "https://huggingface.co/csukuangfj/sherpa-onnx-zipformer-vi-2025-04-20/resolve/main/decoder-epoch-12-avg-8.onnx",
    "models/ASR/zipformer/joiner-epoch-12-avg-8.onnx": "https://huggingface.co/csukuangfj/sherpa-onnx-zipformer-vi-2025-04-20/resolve/main/joiner-epoch-12-avg-8.onnx",
    "models/ASR/zipformer/tokens.txt": "https://huggingface.co/csukuangfj/sherpa-onnx-zipformer-vi-2025-04-20/resolve/main/tokens.txt",
    "models/ASR/zipformer/bpe.model": "https://huggingface.co/csukuangfj/sherpa-onnx-zipformer-vi-2025-04-20/resolve/main/bpe.model",
    "models/TTS/glados.onnx": "https://github.com/dnhkng/GlaDOS/releases/download/0.1/glados.onnx",
    "models/TTS/vi_VN-vais1000-medium.onnx": "https://huggingface.co/rhasspy/piper-voices/resolve/3d796cc2f2c884b3517c527507e084f7bb245aea/vi/vi_VN/vais1000/medium/vi_VN-vais1000-medium.onnx",
    "models/TTS/vi_VN-vais1000-medium.onnx.json": "https://huggingface.co/rhasspy/piper-voices/resolve/3d796cc2f2c884b3517c527507e084f7bb245aea/vi/vi_VN/vais1000/medium/vi_VN-vais1000-medium.onnx.json",
...
def verify_checksums() -> dict[str, bool]:
    ...
    marked as invalid. Files with empty checksums (newly added models) are only checked
    for existence.
    ...
    for path, expected in MODEL_CHECKSUMS.items():
        ...
        # If no expected checksum is provided (empty string), just check existence
        if not expected:
            results[path] = True
            continue
...
def download_models() -> None:
    ...
    The function downloads the following models:
        - ASR models: Zipformer Vietnamese ASR models
        - VAD model: silero_vad.onnx
        - TTS models: glados.onnx, vietnamese.onnx
    ...
    for path, is_valid in checksums.items():
        if not is_valid:
            try:
                download_with_progress(MODEL_URLS[path], resource_path(path), MODEL_CHECKSUMS[path])
